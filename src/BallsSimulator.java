...
//METHODES
	...
	@Override
	public void next() {
		for(Point n:this.b.getpoint()){// on efface les formes affichées aux anciennes coordonnées
			gui.addGraphicalElement(new Oval((int)n.getX(),(int)n.getY(),Color.BLACK,Color.BLACK,12));
			}
    b.translate(10,10);
		//b.reset();
		for(Point n:this.b.getpoint()){// on crée un élement graohique pour chaque point de l'ensemble des balles aux nouvelles coordonnées
			gui.addGraphicalElement(new Oval((int)n.getX(),(int)n.getY(),Color.WHITE,Color.BLACK,10));
			}
		}
	@Override
